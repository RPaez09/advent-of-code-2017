var input = ["104 240 147 246 123 175 372 71 116 230 260 118 202 270 277 292",
...
function checkLine( arg )
{

  var arr = arg.split(' ');

  var clean = []; //array to be populated with numbers

  for( var x = 0; x < arr.length; x++ )
  {
    clean.push( parseInt( arr[ x ] ) ); 
  }
  
  clean.sort( function(a,b) { return b-a } ); // sorted in reverse order
  
  for( var i = 0; i < clean.length; i++)
  {
    for( var k = i + 1; k < clean.length; k++ )
    {
      if( clean[i] % clean[k] === 0 ) // check to see if evely divisible
      {
        return clean[i]/clean[k]
      }
    }
  }
...
function checkSummer( arg )
{
  var ans = 0;
  
  for( var j = 0; j < arg.length; j++ ) // run through each separate line
  {
      ans += checkLine( arg[ j ] ); // add the divided answer
  }
  
  return ans
}

console.log( checkSummer(input) );